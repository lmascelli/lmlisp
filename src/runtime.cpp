          ...
          if (u_ast->size() == 0) {
            ...
            //************************** defmacro! ****************************//
              ...
              if (u_ast->at_least(3) and u_ast->check_nth(1, SYMBOL)) {
                ...
                else THROW("defmacro!: define a function as macro");
              } else
            ...
            // *********************** APPLY SECTION **************************//
              ...
              } else THROW("'" + pr_str(u_ast->at(0)) + "' not found");
            }
          }
        }
      }
    ...
    switch (ast->type) {
      ...
      case DICT: {
                   ...
                   for (unsigned int i = 0; i < keys->size(); i++)
                     ret->append(keys->at(i), 
                         EVAL(ast->to<Dict>()->get(keys->at(i)), env));
                   return ret;
                 }
      default:
                 return ast;
